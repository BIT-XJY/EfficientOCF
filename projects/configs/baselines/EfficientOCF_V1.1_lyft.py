# Developed by Jingyi Xu based on the codebase of Cam4DOcc
...
# we only consider use_camera in the current version
input_modality = dict(
    use_lidar=False,
    use_camera=True,
    use_radar=False,
...
dataset_type = 'EfficientOCFLyftDataset'
...
train_pipeline = [
    dict(type='LoadInstanceWithFlow', ocf_dataset_path=ocf_dataset_path, grid_size=occ_size, use_flow=True, background=empty_idx, pc_range=point_cloud_range,
    ...
    dict(type='LoadOccupancy', to_float32=True, occ_path=occ_path, ocf_dataset_path=ocf_dataset_path, grid_size=occ_size, unoccupied=empty_idx, pc_range=point_cloud_range, use_fine_occ=use_fine_occ, test_mode=False, use_lyft=True),
    dict(type='OccDefaultFormatBundle3D', class_names=class_names),
    dict(type='Collect3D', keys=['img_inputs_seq', 'gt_occ', 'flow_bev', 'future_egomotion', 'height', 'segmentation', 'segmentation_bev', 'instance_bev']),
]
...
test_pipeline = [
    ...
    dict(type='LoadOccupancy', to_float32=True, occ_path=occ_path, ocf_dataset_path=ocf_dataset_path, grid_size=occ_size, unoccupied=empty_idx, pc_range=point_cloud_range, use_fine_occ=use_fine_occ, test_mode=True, use_lyft=True),
    dict(type='OccDefaultFormatBundle3D', class_names=class_names, with_label=False), 
    dict(type='Collect3D', keys=['img_inputs_seq', 'gt_occ', 'flow_bev', 'future_egomotion', 'height', 'segmentation', 'segmentation_bev', 'instance_bev'], meta_keys=['pc_range', 'occ_size', 'scene_token', 'lidar_token']),
]
