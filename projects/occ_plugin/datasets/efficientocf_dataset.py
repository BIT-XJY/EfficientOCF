...
class EfficientOCFDataset(NuScenesDataset):
    def __init__(self, occ_size, pc_range, occ_root, idx_root, ori_data_root, save_local_root, data_root, time_receptive_field, n_future_frames, classes, use_separate_classes,
        ...
        if rank == 0:
            print("-------------")
            print("use " + str(self.time_receptive_field) + " past frames to forecast " + str(self.n_future_frames) + " future frames")
            print("-------------")
        ...
        if self.test_mode:
            ...
            self.chosen_list = random.sample(range(0, 23930) , self.train_capacity)
            self.chosen_list_num = len(self.chosen_list)
    ...
    def prepare_sequential_data(self, index):
        ...
        for key in keys:
            input_seq_data[key] = []
        scene_lidar_token = [] # scene_token + lidar_token
